...
if ~isempty(opt.loadTrans) && ~isempty(opt.loadTransYearStart) && ~isempty(opt.loadTransYearEnd)
    fprintf(fid, 'loadTrans %s %d %d\n', opt.loadTrans,...
        opt.loadTransYearStart, opt.loadTransYearEnd);
end
